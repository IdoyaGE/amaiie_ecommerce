...
export default function OrderScreen() {
    ...
    return actions.order.capture().then(async function (details) {
      try {
        ...
        const { data } = await axios.put(
          `https://api.amaiie.miratuweb.online/api/orders/${order._id}/pay`,
          details,
          {
    ...
    const fetchOrder = async () => {
      try {
        ...
        const { data } = await axios.get(
          `https://api.amaiie.miratuweb.online/api/orders/${orderId}`,
          {
            headers: { authorization: `Bearer ${userInfo.token}` },
          }
        );
    ...
    if (
      ...
      const loadPaypalScript = async () => {
        const { data: clientId } = await axios.get(
          "https://api.amaiie.miratuweb.online/api/keys/paypal",
          {
            headers: { authorization: `Bearer ${userInfo.token}` },
          }
        );
  ...
  async function deliverOrderHandler() {
    try {
      ...
      const { data } = await axios.put(
        `https://api.amaiie.miratuweb.online/api/orders/${order._id}/deliver`,
        {},
        {
          headers: { authorization: `Bearer ${userInfo.token}` },
        }
                ...
                {order.orderItems.map((item) => (
                  <ListGroup.Item key={item._id}>
                    <Row className='align-items-center'>
                      <Col md={6}>
                        <img
                          src={`https://api.amaiie.miratuweb.online/fotoproducto/${item.image}`}
                          alt={item.name}
